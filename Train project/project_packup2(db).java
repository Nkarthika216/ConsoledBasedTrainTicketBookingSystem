// compile   :  javac -cp "C:\Users\Lenovo\Downloads\postgresql-42.6.0.jar;" -d . BookingWithDB.java
// run       :  java -cp "C:\Users\Lenovo\Downloads\postgresql-42.6.0.jar;" TrainTicketBookingProject.BookingWithDB    
...
import DbOperations.TrainTicketBookingUtil;
...
public static boolean isValidCardNumber(String card_no){
    ...
    public static void main(String args[])throws ParseException{
        ...
        TrainTicketBookingUtil utilObj=new TrainTicketBookingUtil();
        // utilObj.insertTrainDetails(trains);
        userdetails.put("karthi@gmail.com",new User("karthi","21-06-2000","karthi@gmail.com","Nkarthi@123","9952857934"));
        ...
        while(flag){
            ...
            switch(choice){
               ...
               loop=isValidPassword(password);
                ...
                String dobString=dob.split("-")[2]+"-"+dob.split("-")[1]+"-"+dob.split("-")[0];
                System.out.println("dob string:  "+dobString);
                boolean signUpStatus=utilObj.signUp(new User(name,dobString,mail_id,password,phone_no));
                if(signUpStatus)
               ...
               loop=isValidPassword(pwd);
                ...
                boolean login_status=utilObj.login(mail,pwd);
                if(login_status)
                {
                    System.out.println();
...
return al;
    ...
    public static ArrayList<TrainDetails> addTrains(){
        ...
        return trains;
    }//create table coach(trainid int not null references train(id),name varchar(10) not null,
}
...
return signUpStatus;
    ...
    // public boolean findAvailableTrains(java.sql.Date date,String from,String to){
    //     try {
    ...
    public void insertTrainDetails(ArrayList<TrainDetails> trains){
        ...
        connection=DbConnection.getInstance().getConnection();
            for(TrainDetails train:trains){
        preparedStatement=connection.prepareStatement("INSERT INTO train(name,number,rate_per_km) values(?,?,?) returning id");
        ...
        boolean[] seatStatusArr=null;
        // int a=null;
                // String query="INSERT INTO coach(trainid,name,class_type_id,number_of_seats) values";
                // String values="";
                for(Coach coach:train.coachs){
                    int class_type_id=-1;
                    preparedStatement=connection.prepareStatement("SELECT INTO classtype where type=?");
                    preparedStatement.setString(1,coach.class_type);
                    resultSet=preparedStatement.executeQuery();
                    if(resultSet.next())
                    class_type_id=resultSet.getString("id");
                    else{
                        preparedStatement=connection.prepareStatement("INSERT INTO classtype where type=? returning id");
                        preparedStatement.setString(1,coach.class_type);
                    ...
                    for(int i=1;i<=coach.seat_count;i++){
                        ...
                        berthId=4;
                        preparedStatement=connection.prepareStatement("INSERT INTO seat(number,trainid,coachid,berthid) values(?,?,?,?) returning id");
